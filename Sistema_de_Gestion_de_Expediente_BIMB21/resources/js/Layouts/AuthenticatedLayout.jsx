...
export default function AuthenticatedLayout({ header, children }) {
  ...
  // ✅ Obtener permisos del usuario (ASEGÚRATE de pasar esto desde el backend)
  ...
  useEffect(() => {
    ...
    // Mapeo de rutas a IDs del menú (USANDO LAS RUTAS QUE REALMENTE EXISTEN)
    const routeToMenuId = {
      '/dashboard': 'dashboard',
      '/personas': 'personas',
      '/expedientes': 'expedientes', // Agregado del remoto
      '/reportes': 'reportes', // Agregado del remoto
      '/usuarios': 'usuarios', // Agregado del remoto
      '/backups': 'backups',
      '/asistencias': 'asistencias', // De tu versión
      '/jerarquias': 'jerarquias', // De tu versión
      '/permisos': 'permisos', // De tu versión (ojo: esto es para roles)
    };

    // Encontrar el ID del menú basado en la ruta actual
    const matchedRoute = Object.keys(routeToMenuId).find(route => currentPath.startsWith(route));
  ...
  // ✅ Menú de navegación DINÁMICO CON PERMISOS
  const menuItems = [
    {
      id: 'dashboard',
      ...
      href: '/dashboard',
      exact: true,
      permission: 'system.view', // O cualquier permiso para dashboard
    },
    {
      id: 'personas',
      label: 'Personal',
      icon: Users,
      href: '/personas',
      permission: 'personas.view', // SOLO se muestra si tiene este permiso
    },
    {
      id: 'expedientes',
      label: 'Expedientes',
      icon: Folder,
      href: '/expedientes',
      permission: 'expedients.view', // Del remoto
    },
    {
      id: 'asistencias',
      label: 'Asistencias',
      icon: Calendar,
      href: '/asistencias',
      permission: null, // Si no requiere permiso específico
    },

    {
      id: 'usuarios', // Del remoto (Gestión de usuarios)
      label: 'Usuarios',
      icon: UserCog,
      href: '/usuarios',
    ...
    {
      id: 'permisos', // ← Cambia de 'usuarios' a 'permisos'
      label: 'Roles y Permisos', // ← Cambia el nombre
      icon: UserCog,
      href: '/permisos', // ← Cambia de '/usuarios' a '/permisos'
  ...
  const filteredMenuItems = menuItems.filter(item => {
    ...
    // O si es administrador (tiene todos los permisos)
  ...
  return (
    <div className="d-flex min-vh-100 bg-light">
      ...
      >
        ...
        <div className="d-flex align-items-center justify-content-between p-3 border-bottom border-secondary">
          {!sidebarCollapsed && (
            <Link href="/" className="d-flex align-items-center text-decoration-none text-white">
              <ApplicationLogo className="me-2" style={{ height: '32px' }} />
              <span className="fs-5 fw-bold">Menu</span>
            </Link>
          )}
          {sidebarCollapsed && (
            <Link href="/" className="d-flex justify-content-center w-100">
              <ApplicationLogo className="text-white" style={{ height: '32px' }} />
            </Link>
          )}
          <button
            ...
            {filteredMenuItems.map(item => (
              <li key={item.id} className="nav-item">
                ...
                >
                  ...
                  {!sidebarCollapsed && <span className="ms-3 flex-grow-1">{item.label}</span>}
                </Link>
              </li>
            ))}
          </ul>
        ...
        {/* Perfil de Usuario con Dropdown (TU VERSIÓN - mejor) */}
            ...
            {!sidebarCollapsed && (
              <>
                <div className="flex-grow-1 ms-3 min-w-0">
                  <p className="mb-0 fw-medium text-truncate">{user.name}</p>
          ...
          <header className="bg-white shadow-sm border-bottom">
            <div className="container-fluid py-3">
              <div className="d-flex align-items-center justify-content-between">
                <div>
                  <h1 className="h3 mb-0 fw-bold text-dark">{header}</h1>
